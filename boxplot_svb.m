...
for i = 1:length(list)
  ...
  DE = [DE; dwellt_all_exp];	
  CE = [CE; ncurr_all_exp];
  
  % Determine number of events in this data set.
  SE(i) = length(dwellt_all_exp);
  ...
  DET = [DET; dwellt_tcz_exp];
  CET = [CET; ncurr_tcz_exp];
  
  % Determine number of events in this data set.
  SET(i) = length(dwellt_tcz_exp);    
...
title('Experiment - Dwell Time - All Filtered Events');
xlabel('Tagged Nucleotides');
ylabel('Dwell Time (s)');
savefig('box_dwell_all_exp.fig');
print('-dbmp', 'box_dwell_all_exp.bmp'); 
disp('--> Boxplot created for all filtered events (exp)');

% Create normalized current (%) boxplot of all filtered events (exp).
figure(2);
boxplot(CE, GE); 
set(gca, 'ylim', [0.0 0.7001]);
title('Experiment - Normalized Current Blockade Level (%) - All Filtered Events');
xlabel('Tagged Nucleotides');
ylabel('Normalized Current Blockade Level (%)');
savefig('box_ncurr_all_exp.fig');
print('-dbmp', 'box_ncurr_all_exp.bmp'); 
disp('--> Boxplot created for all filtered events (exp)');
...
% Create dwell time (s) boxplot of tag capture zone events (exp).
figure(5);
boxplot(DET, GET);
set(gca, 'YScale', 'log', 'ylim', [1E-3 1E+3]);
title('Experiment - Dwell Time - Tag Capture Zone');
xlabel('Tagged Nucleotides');
ylabel('Dwell Time (s)');
savefig('box_dwell_tcz_exp.fig');
print('-dbmp', 'box_dwell_tcz_exp.bmp'); 
disp('--> Boxplot created for tag capture zone events (exp)');

% Create normalized current (%) boxplot of tag capture zone events (exp).
figure(6);
boxplot(CET, GET); 
set(gca, 'ylim', [0.0 0.7001]);
title('Experiment - Normalized Current Blockade Level (%) - Tag Capture Zone');
xlabel('Tagged Nucleotides');
ylabel('Normalized Current Blockade Level (%)');
savefig('box_ncurr_tcz_exp.fig');
print('-dbmp', 'box_ncurr_tcz_exp.bmp'); 
disp('--> Boxplot created for tag capture zone events (exp)');
