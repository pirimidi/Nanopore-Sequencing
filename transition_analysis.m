...
function [S] = transition_analysis(filter_e, order_e, tag_capture, dwell_cutoff)
...
fprintf('\n');
disp('--> Transition analysis start');
fprintf('\n');
...
m = matfile('transition_9.mat', 'Writable', true);
...
XX = logspace(-3, 3, 1e+2);
% XE = logspace(-3, 3, 0.5e+2);
XE = 0.5e+2;
...
% figure(1);
% histogram(WE, XE, 'Normalization', 'probability');
...
% figure(2);
% histogram(DE, XE, 'Normalization', 'probability');
...
figure(4);
histogram(W_NN, XE, 'Normalization', 'probability');
title('N->N state transition wait time frequency');
xlabel('Wait Time (s)');
ylabel('Frequency (cnt)');
grid;
%axis([1e-3 1e+3 0 0.1]);
%set(gca,'xscale','log') 
savefig('NN_wait.fig');
...
figure(5);
histogram(D_NN, XE, 'Normalization', 'probability');
...
figure(7);
histogram(W_NM, XE, 'Normalization', 'probability');
...
figure(8);
histogram([D_GA D_GC D_GT D_AG D_AC D_AT D_CG D_CA D_CT D_TG D_TA D_TC], XE, 'Normalization', 'probability');
...
figure(9);
hist3([W_NM', ...
       [D_GA D_GC D_GT D_AG D_AC D_AT D_CG D_CA D_CT D_TG D_TA D_TC]'], {XX XX});
...
%             figure(10+3*(f-1));
%             histogram(W{k}, XE, 'Normalization', 'probability');
...
%             figure(11+3*(f-1));
%             histogram(D{k}, XE, 'Normalization', 'probability');
